...
// Define the wave animation using keyframes
const waveAnimation = `
@keyframes wave {
  0% {
    transform: scale(0);
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <>
        <style>{waveAnimation}</style> {/* Inject the keyframes into the document */}
        <Comp
          className={cn(
            buttonVariants({ variant, size, className }),
            "hover:animate-wave" // Add hover animation class here
          )}
          ref={ref}
          {...props}
        />
